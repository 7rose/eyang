...
class Link
{
    ...
    {
        $templet = $this->getTemplet($org_id);
        $param = $this->getParam($org_id, $url);
        $product_code = $this->getProductCode($org_id, $url);

        $engine = new Engine;

        $product_templet = $engine->render($templet, ['param' => $param, 'product' => $product_code]);
        return $product_templet;
    }
     ...
     *
     */
    public function getTemplet($org_id)
    {
        $org = Org::findOrFail($org_id);

        $templet = $this->getCode($org->config, 'templet');
     ...
     *
     */
    public function getParam($org_id, $url)
    {
    ...
    public function getMatch($org_id, $url)
    {
        ...
        if(!count($array['shop']) || !count($array['product'] || !count($array['param'])) return false;
        return $array;
    }
